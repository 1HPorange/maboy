...
impl CPU {
    ...
    fn execute<CRAM: CartridgeRam>(&mut self, board: &mut Board<CRAM>, instr: ByteInstr) {
        ...
        match instr {
            ...
            RRA => rra(self, board),
            JR_NZ_r8 => jr_cond(self, board, !self.reg[Flags::Z]),
            ...
            DAA => daa(self, board),
            JR_Z_r8 => jr_cond(self, board, self.reg[Flags::Z]),
            ...
            CPL => cpl(self),
            JR_NC_r8 => jr_cond(self, board, !self.reg[Flags::C]),
            ...
            SCF => scf(self),
            JR_C_r8 => jr_cond(self, board, self.reg[Flags::C]),
            ...
            RET_NZ => ret_cond(self, board, !self.reg[Flags::Z]),
            POP_BC => pop(self, board, BC),
            JP_NZ_a16 => jp_cond(self, board, !self.reg[Flags::Z]),
            JP_a16 => jp_cond(self, board, true),
            CALL_NZ_a16 => call_cond(self, board, !self.reg[Flags::Z]),
            PUSH_BC => push(self, board, BC),
            ADD_A_d8 => add8(self, board, Imm8),
            RST_00H => rst(self, board, 0x00),
            RET_Z => ret_cond(self, board, self.reg[Flags::Z]),
            RET => ret(self, board, false),
            JP_Z_a16 => jp_cond(self, board, self.reg[Flags::Z]),
            PREFIX_CB => self.fetch_execute_cb(board),
            CALL_Z_a16 => call_cond(self, board, self.reg[Flags::Z]),
            CALL_a16 => call_cond(self, board, true),
            ADC_A_d8 => adc8(self, board, Imm8),
            RST_08H => rst(self, board, 0x08),
            RET_NC => ret_cond(self, board, !self.reg[Flags::C]),
            POP_DE => pop(self, board, DE),
            JP_NC_a16 => jp_cond(self, board, !self.reg[Flags::C]),
            NOT_USED => self.set_halt_state(HaltState::Stuck),
            CALL_NC_a16 => call_cond(self, board, !self.reg[Flags::C]),
            PUSH_DE => push(self, board, DE),
            SUB_d8 => sub8(self, board, Imm8),
            RST_10H => rst(self, board, 0x10),
            RET_C => ret_cond(self, board, self.reg[Flags::C]),
            RETI => ret(self, board, true),
            JP_C_a16 => jp_cond(self, board, self.reg[Flags::C]),
            NOT_USED_0 => self.set_halt_state(HaltState::Stuck),
            CALL_C_a16 => call_cond(self, board, self.reg[Flags::C]),
            ...
            LD_HL_SPpr8 => unimplemented!(),
            LD_SP_HL => unimplemented!(),
